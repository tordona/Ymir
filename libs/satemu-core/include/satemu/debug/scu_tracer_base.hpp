...
struct ISCUTracer {
    ...
    // `pc` is the Program RAM address of the DMA instruction.
    virtual void BeginDSPDMA(bool toD0, uint32 addrD0, uint8 addrDSP, uint8 count, uint8 addrInc, bool hold, uint8 pc) {
    }

    // Invoked on every DSP DMA transfer.
    //
    // `addrD0` is the D0 bus address.
    // `offsetDSP` is the offset into the selected DSP RAM bank.
    // `value` is the transferred value.
    virtual void DSPDMATransfer(uint32 addrD0, uint8 offsetDSP, uint32 value) {}

    // Invoked when a DSP DMA transfer ends.
    virtual void EndDSPDMA() {}
};
