...
struct VDP1Params {
    ...
    // Bit depth selection - 8bpp (true) or 16bpp (false)
    ...
    void UpdateTVMR() {
        ...
        const uint8 tvm = (hdtvEnable << 2) | (fbRotEnable << 1) | bitDepth8;
        fbSizeH = kSizesH[tvm];
        fbSizeV = kSizesV[tvm];
    }
};
...
struct VDP1Regs {
    ...
    FORCE_INLINE void WriteTVMR(uint16 value) {
        ...
        params.bitDepth8 = bit::extract<0>(value);
        params.UpdateTVMR();
    }
    ...
    FORCE_INLINE uint16 ReadMODR() const {
        uint16 value = 0;
        bit::deposit_into<0>(value, params.bitDepth8);
