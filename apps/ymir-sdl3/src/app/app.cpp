...
void App::RunEmulator() {
    ...
    m_context.saturn.VDP.SetRenderCallback(
        {&m_context, [](uint32 *fb, uint32 width, uint32 height, void *ctx) {
             ...
             if (screen.videoSync && screen.skipEmuFrameCounter + 1 >= screen.skipEmuFrames) {
    ...
    while (true) {
        ...
        if (m_context.emuSpeed.limitSpeed) {
            ...
            if (baseFrameInterval > kMaxGUIFrameInterval) {
                // Duplicate frames displayed by the GUI to maintain a minimum GUI frame rate
                screen.dupGUIFrames = std::ceil(baseFrameInterval / kMaxGUIFrameInterval);
                ...
                if (dispMode != nullptr && dispMode->refresh_rate != 0.0f) {
                    ...
                    screen.skipEmuFrames = std::ceil(kMaxEmuFrameInterval / baseFrameInterval);
        ...
        if (screen.updated || screen.videoSync) {
            if (screen.videoSync && screen.expectFrame && ++screen.skipEmuFrameCounter >= screen.skipEmuFrames) {
