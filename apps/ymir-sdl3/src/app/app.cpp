...
void App::RunEmulator() {
    ...
    m_context.saturn.VDP.SetRenderCallback(
        {&m_context, [](uint32 *fb, uint32 width, uint32 height, void *ctx) {
             ...
             if (sharedCtx.emuSpeed.limitSpeed && screen.videoSync &&
                 screen.skipEmuFrameCounter >= screen.skipEmuFrames) {
                 screen.frameRequestEvent.Wait(true);
                 screen.skipEmuFrameCounter = 0;
             }
    ...
    while (true) {
        ...
        if (m_context.emuSpeed.limitSpeed) {
            ...
            // Don't let GUI go below 48 fps; duplicate frames if needed
            static constexpr double kMinGUIFrameRate = 48.0;
            static constexpr auto kMaxGUIFrameInterval =
                std::chrono::duration_cast<std::chrono::nanoseconds>(1s / kMinGUIFrameRate);
            ...
            if (baseFrameInterval > kMaxGUIFrameInterval) {
                ...
                screen.dupGUIFrames = std::max(1.0, std::ceil(baseFrameInterval / kMaxGUIFrameInterval));
                baseFrameInterval /= screen.dupGUIFrames;
            } else {
                screen.dupGUIFrames = 1;
            }

            // Skip frames generated by the emulator to stay under the display's refresh rate
            {
                ...
                if (dispMode != nullptr && dispMode->refresh_rate != 0.0f) {
                    const auto kMaxEmuFrameInterval =
                        std::chrono::duration_cast<std::chrono::nanoseconds>(1s / dispMode->refresh_rate);
                    screen.skipEmuFrames = std::max(1.0, std::ceil(kMaxEmuFrameInterval / baseFrameInterval));
                    baseFrameInterval *= screen.skipEmuFrames;
                } else {
                    // No refresh rate info, sadly
                    screen.skipEmuFrames = 1;
                }
            }
            const auto gcd = std::gcd(screen.dupGUIFrames, screen.skipEmuFrames);
            screen.dupGUIFrames /= gcd;
            screen.skipEmuFrames /= gcd;
    ...
    while (true) {
        ...
        if (!paused) {
            ...
            if (m_context.emuSpeed.limitSpeed && m_context.screen.videoSync &&
                ++m_context.screen.skipEmuFrameCounter >= m_context.screen.skipEmuFrames) {
